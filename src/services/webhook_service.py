...
class WebhookService:
    ...
    async def notify(
        ...
        if event_type not in webhook.event_types and WebhookEventType.ALL not in webhook.event_types:
            logger.debug(f"Event type {event_type} not subscribed by webhook {webhook.id}")
            return False
