...
def test_data_extraction_rule():
    ...
    rule = DataExtractionRule(name="order_number", pattern=r"Order #: (\d+)")
...
def test_extraction_rule_content_type():
    ...
    text_rule = DataExtractionRule(
        name="amount", pattern=r"Amount: \$([\d,.]+)", content_type="text"
    )
    ...
    html_rule = DataExtractionRule(
        name="amount", pattern=r"Amount: \$([\d,.]+)", content_type="html"
    )
    ...
    both_rule = DataExtractionRule(
        name="amount", pattern=r"Amount: \$([\d,.]+)", content_type="both"
    )
...
def test_filter_with_extraction_rules():
    ...
    extraction_rules = [
        DataExtractionRule(name="order_number", pattern=r"Order #: (\d+)"),
        DataExtractionRule(name="total_amount", pattern=r"Total: \$(\d+\.\d+)"),
    ]
